...
st.markdown('必要サンプル数')
bottom, up = sp.stats.norm.interval(alpha=alpha, n=visitors_a, p=conversion_a/visitors_a, loc=0, scale=1)
st.markdown(f'<center><font size=7 color="#FF4B00"> {(2*up*np.sqrt(p*(1-p)))/sample}</font></center>', unsafe_allow_html=True)
